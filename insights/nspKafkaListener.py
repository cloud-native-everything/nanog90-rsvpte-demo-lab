...
from insights.nsp_multiPccLspPaths import _multiLspMgmt_create
from insights.nsp_multiPccLspPaths import _multiLspMgmt_delete
...
class KafkaEventListener:
    def __init__(self, bootstrap_servers, topic, partition, ssl_ca_location, upthreshold, downthreshold, config, data, qty,debug):
...
## Process Messages from LSP_USAGE
    ...
    def lsp_usage_process_message(self, message_value, name):
     ...
     # Check if the counter exceeds the threshold
        ...
        if current_time - self.window_start_time >= timedelta(seconds=TIME_PERIOD):
            ...
            if len(self.timestamps_over_threshold) > OCCURRENCES:
                print(f"{current_time} - INFO: [Up]Threshold [Ex]ceeded more than {OCCURRENCES} times in the last {TIME_PERIOD} seconds! Triggering event...")
                self.lsp_usage_trigger_clone_creation()
            if len(self.timestamps_under_threshold) > OCCURRENCES:
                print(f"{current_time} - INFO: [Down]Threshold [Sub]ceeded more than {OCCURRENCES} times in the last {TIME_PERIOD} seconds! Triggering event...")
